...
public class httpServerDecoder implements ProtocolDecoder{
	...
	public void decode(IoSession session, IoBuffer msg, ProtocolDecoderOutput out){
			...
			switch(state){
 			...
 			case NEW:
 					httpRequest rq =  parseHttpRequestHead(msg.buf());
 					if (rq == null )
 					{
	...
	private httpRequest parseHttpRequestHead(final ByteBuffer buffer){
			...
			String[] fileNameAndFileContent = CONTENT_BODY_PATTERN.split(headerAndBody[1]);
			fileNameAndFileContent = ArrayUtil.dropFromEndWhile(fileNameAndFileContent,"");
			final String fileName = fileNameAndFileContent[0];
			final String fileInString = fileNameAndFileContent[1];
			...
			return new httpRequest(version, method, requestedPath, queryString, 
						generalHeaders, fileName, fileInString);
